...
export class WhatsAppBotService {
  ...
  private async manejarMenuPrincipal(telefono: string, mensaje: string, contexto: ConversationContext, conversacionId: string) {
    if (mensaje === 'menu_pedir') {
      ...
      if (cliente?.referidoPorId) {
        await mensajeriaService.enviarMensaje(telefono, MENSAJES.INFO_REFERIDOS_YA_REGISTRADO());
        await this.enviarMenuPrincipal(telefono);
  ...
  private async procesarEstado(
    ...
    switch (estado) {
      ...
      case 'MENU_PRINCIPAL':
        await this.manejarMenuPrincipal(telefono, mensaje, contexto, conversacionId); break;
  ...
  // dirección no cambia lo que Mapbox puede resolver para ella.
  private registrarIntentoFallido(contexto: ConversationContext, campo: 'recogida' | 'destino'): number {
  ...
  private async manejarDireccion(
    ...
    if (ubicacion) {
      const lugarBuscado = [ubicacion.nombre, ubicacion.direccion].filter(Boolean).join(' - ');
      const direccionFormateada =
        lugarBuscado ||
        (await mapboxService.reverseGeocodificar(ubicacion.lat, ubicacion.lng)) ||
        `Ubicación compartida (${ubicacion.lat.toFixed(5)}, ${ubicacion.lng.toFixed(5)})`;
      contexto[campo] = { lat: ubicacion.lat, lng: ubicacion.lng, direccionFormateada };
